...
async function main() {
    ...
    if (urlIndex === -1 || objectiveIndex === -1) {
        logger.error("❌ Missing required arguments. Usage: npm start -- --url <URL> --objective <OBJECTIVE>");
        process.exit(1);
    }
    ...
    try {
        ...
        const explorer = new WebExplorer(browser, page, objective, startUrl, stagehand);
        // Start tool-driven exploration
        const success = await explorer.explore();
        if (success) {
            logger.info("✅ Exploration completed successfully - Objective achieved!");
        }
        else {
            logger.info("⚪ Exploration completed - Objective not achieved, but data collected");
        }
    }
    catch (error) {
