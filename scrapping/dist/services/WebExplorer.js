...
export class WebExplorer {
    ...
    llmClient;
    stagehand;
    sessionDecisionHistory = []; // Track all decisions made during the session
    isExplorationObjective; // Add exploration mode detection
    activeProcessingPromises = new Map(); // Track background processing
    pageLinkages = new Map(); // Track which page leads to which pages
    maxPagesToExplore; // Maximum pages to explore
    socket;
    userName;
    activeExplorations; // Reference to activeExplorations from SocketServer
    additionalContext;
    canLogin;
    constructor(browser, page, objective, startUrl, stagehand, maxPagesToExplore = 6, socket, userName, activeExplorations, isExploration = false, additionalContext, canLogin = false) {
        ...
        this.llmClient = new LLMClient(this.fileManager, this.additionalContext, this.canLogin);
        // Initialize session
        this.session = {
            metadata: {
                sessionId,
     ...
     */
    async explore(maxPagesToExplore) {
     ...
     */
    async waitForAllProcessingToComplete() {
        if (this.activeProcessingPromises.size > 0) {
    ...
    async processPage(pageData) {
        ...
        try {
            ...
            const initialScreenshotPath = this.fileManager.saveScreenshot(pageData.urlHash, 0, // Step 0 for initial page load
            "initial_page_load", screenshotBuffer);
            ...
            while (stepsOnThisPage < maxStepsPerPage &&
                !this.session.metadata.objectiveAchieved) {
                // Check if user is still active before each step
                if (!this.isUserStillActive()) {
                    logger.info(`🛑 Stopping page processing loop - user no longer active`);
                    return;
                }
                ...
                const maxPagesReached = this.session.metadata.totalPagesDiscovered >= this.maxPagesToExplore;
                // Check if user is still active before LLM call
                if (!this.isUserStillActive()) {
                    logger.info(`🛑 Stopping before LLM decision - user no longer active`);
                    return;
                }
                const decision = await this.llmClient.decideNextAction(currentScreenshotBuffer, pageData.url, this.session.metadata.objective, pageData.urlHash, this.session.globalStepCounter, conversationHistory, this.session.pageQueue, this.session.pages, this.isExplorationObjective, maxPagesReached, this.session.userInputs, this.session.flowContext, this.session.actionHistory);
                ...
                logger.info(`🤖 LLM Decision: ${decision.tool_to_use}`, {
                    ...
                    instruction: decision.tool_parameters.instruction,
                    nextPlan: decision.next_plan,
                    isCurrentPageExecutionCompleted: decision.isCurrentPageExecutionCompleted,
                });
                ...
                if (decision.tool_to_use === "user_input") {
                    ...
                    stepResult = await this.executeUserInputTool(decision.tool_parameters, pageData, currentScreenshotBuffer);
                }
                else {
                    stepResult = await this.executeTool(decision.tool_to_use, decision.tool_parameters.instruction, pageData, currentScreenshotBuffer, decision.tool_parameters.inputKey, decision.tool_parameters.inputType, decision.tool_parameters.inputPrompt, decision.tool_parameters.sensitive, decision.tool_parameters.waitTimeSeconds);
                }
                // Don't count standby or sensitive flow steps towards page limits
                if (!decision.isInSensitiveFlow && decision.tool_to_use !== "standby") {
                    stepsOnThisPage++;
                }
                ...
                if (stepResult.tool_used === "page_act" && !stepResult.newUrl) {
                    this.fileManager.saveScreenshot(pageData.urlHash, this.session.globalStepCounter, "before_decision", currentScreenshotBuffer);
                    const afterToolScreenshotBuffer = await this.page.screenshot({
                        fullPage: false,
                    });
                    // Save screenshot after tool execution
                    const afterToolScreenshotPath = this.fileManager.saveScreenshot(pageData.urlHash, this.session.globalStepCounter, `after_${decision.tool_to_use}`, afterToolScreenshotBuffer);
                    // Store screenshot in page data
                    pageData.screenshots.push({
                        stepNumber: this.session.globalStepCounter,
                        timestamp: new Date().toISOString(),
                    ...
                    logger.info(`📸 Post-tool screenshot saved: ${afterToolScreenshotPath}`);
                }
            ...
            this.fileManager.saveSessionConversationHistory(this.sessionDecisionHistory);
     ...
     */
    async executeActionAndCheckUrlChange(instruction) {
     ...
     */
    async toolPageAct(instruction) {
        console.log(`🎬 Acting: ${instruction}`);
        try {
            console.log(`🔧 Calling stagehand.page.act with instruction: "${instruction}"`);
    ...
    Do not ask follow up questions, the user will trust your judgement.`,
            ...
            // });
            if (!result.success) {
                throw new Error(`Action failed for instruction ${instruction}: ${JSON.stringify(result, null, 2)}`);
            }
    ...
    async toolPageExtract(instruction) {
        console.log(`🔍 Extracting: ${instruction}`);
        try {
            // Enhanced schema for more comprehensive analysis
            const extractSchema = z.object({
                extracted_data: z.string().describe("Comprehensive analysis of the page content, features, and functionality. Focus on business value, user experience patterns, technical capabilities, and strategic insights rather than just listing visible elements. Analyze what you see in context of the overall platform strategy."),
                elements_found: z
                    .array(z.string())
                    .describe("Key functional elements and features that indicate platform capabilities and user experience design"),
                page_structure: z
                    .string()
                    .describe("Analysis of page architecture, information hierarchy, and UX design patterns used"),
                interactive_elements: z
                    .array(z.string())
                    .describe("Interactive features and their strategic purpose in the user journey and conversion funnel"),
                business_insights: z
                    .string()
                    .describe("Business model indicators, monetization clues, competitive positioning signals, and strategic market approach evident on this page"),
                technical_observations: z
                    .string()
                    .describe("Technical architecture insights, scalability indicators, integration capabilities, and platform sophistication level"),
            });
...
Provide comprehensive insights rather than just listing visible elements. Analyze WHY features exist and HOW they contribute to the platform's business objectives.`;
            console.log(`🔧 Calling stagehand.page.extract with enhanced instruction`);
     ...
     */
    async toolUserInput(inputs, // Array of InputRequest objects
    singleInputKey, singleInputType, singleInputPrompt) {
        if (!this.socket || !this.userName) {
            return {
                success: false,
                error: "Socket connection or userName not available for input collection",
            };
        }
     ...
     */
    async executeUserInputTool(toolParameters, pageData, screenshotBuffer) {
        // Check if user is still active before tool execution
        if (!this.isUserStillActive()) {
            logger.info(`🛑 Stopping user input tool execution - user no longer active`);
            return null;
        }
        const step = {
        ...
        try {
            ...
            if (toolParameters.inputs && Array.isArray(toolParameters.inputs)) {
                // Multiple inputs mode
                inputResult = await this.toolUserInput(toolParameters.inputs, undefined, // singleInputKey
                undefined, // singleInputType
                undefined // singleInputPrompt
                );
            }
            else if (toolParameters.inputKey &&
                toolParameters.inputType &&
                toolParameters.inputPrompt) {
                // Single input mode (backward compatibility)
                inputResult = await this.toolUserInput([], // Empty array for single input mode
                toolParameters.inputKey, toolParameters.inputType, toolParameters.inputPrompt);
            }
            else {
                return {
                    ...step,
                    success: false,
                    result: "Invalid user input parameters: missing inputs array or single input fields",
                };
            }
                ...
                for (const [key, value] of Object.entries(inputResult.inputs)) {
                    ...
                    this.session.userInputs.set(key, {
                        ...
                        type: toolParameters.inputs
                            ? toolParameters.inputs.find((inp) => inp.inputKey === key)
                                ?.inputType || "text"
     ...
     */
    async toolStandby(waitTimeSeconds, instruction) {
        console.log(`⏳ Standby: ${instruction} - waiting ${waitTimeSeconds} seconds`);
        try {
     ...
     * Execute the chosen tool
     */
    async executeTool(toolName, instruction, pageData, screenshotBuffer, inputKey, inputType, inputPrompt, sensitive, waitTimeSeconds) {
        // Check if user is still active before tool execution
        if (!this.isUserStillActive()) {
        ...
        try {
            switch (toolName) {
                case "page_extract":
                    ...
                    // Check if user is still active after tool execution
                    if (!this.isUserStillActive()) {
                        logger.info(`🛑 Stopping after page_extract - user no longer active`);
                        return null;
                    }
                    if (extractResult.success) {
                        ...
                        const screenshotBuffers = pageData.screenshots.map(s => s.buffer).filter(Boolean);
                        // 3. Collect previous extraction data from current page only
                        const previousExtractions = pageData.extractionResults.map(r => r.rawData);
                        const previousMarkdowns = pageData.extractionResults.map(r => r.formattedMarkdown);
                        ...
                        if (!this.isUserStillActive()) {
                            logger.info(`🛑 Stopping before comprehensive formatting - user no longer active`);
                            return null;
                        }
                        // 4. 🆕 NEW LLM CALL: Comprehensive formatting with all context
                        const formattedMarkdown = await this.llmClient.formatExtractionResults(screenshotBuffers, pageData.url, this.session.metadata.objective, pageData.urlHash, this.session.globalStepCounter, previousExtractions, previousMarkdowns, extractResult.data // Current extraction data
                        );
                        // Check if user is still active after formatting call
                        if (!this.isUserStillActive()) {
                            logger.info(`🛑 Stopping after formatting - user no longer active`);
                            return null;
                        }
                        ...
                        const extractionResult = {
                            ...
                            formattedMarkdown: formattedMarkdown || `# Extraction v${currentVersion}\n\n${JSON.stringify(extractResult.data, null, 2)}`,
                            stepNumber: this.session.globalStepCounter,
                        };
                        ...
                        if (!this.isExplorationObjective) {
                            const analysisResult = await this.llmClient.executePageExtract(screenshotBuffer, pageData.url, instruction, this.session.metadata.objective, pageData.urlHash, this.session.globalStepCounter, extractResult);
                            if (analysisResult) {
                        ...
                        if (!this.isUserStillActive()) {
                            logger.info(`🛑 Stopping after objective check - user no longer active`);
                            return null;
                        }
                        ...
                        if (this.socket && this.userName) {
                            this.socket.emit("exploration_update", {
                                ...
                                data: {
                                    ...
                                    interactiveElements: extractResult.data?.interactive_elements || [],
                                },
                            });
                        }
                        logger.info(`📊 Enhanced page_extract completed v${currentVersion}`, {
                            dataExtracted: true,
                    ...
                    break;
                case "page_act":
                    const actionResult = await this.executeActionAndCheckUrlChange(instruction);
                    // Check if user is still active after tool execution
                    if (!this.isUserStillActive()) {
                    ...
                    if (discoveredUrl) {
                        ...
                        if (this.session.flowContext.isInSensitiveFlow) {
                            ...
                            logger.info(`🔒 Sensitive flow detected - staying on new URL without queuing: ${discoveredUrl}`);
                    ...
                    if (toolResult.success) {
                        // Check if user is still active before LLM call
                        if (!this.isUserStillActive()) {
                            logger.info(`🛑 Stopping before LLM call - user no longer active`);
                            return null;
                        }
                        if (!this.isExplorationObjective) {
                            const analysisResult = await this.llmClient.executePageAct(screenshotBuffer, // Use the screenshot provided to executeTool
                            pageData.url, instruction, this.session.metadata.objective, pageData.urlHash, this.session.globalStepCounter, toolResult);
                            if (analysisResult) {
                                step.objectiveAchieved = analysisResult.objectiveAchieved;
                            }
                        }
                ...
                case "user_input":
                    ...
                    const inputResult = await this.toolUserInput([], // Empty array for single input mode
                    inputKey, inputType, inputPrompt);
                    // Check if user is still active after tool execution
                    if (!this.isUserStillActive()) {
                        logger.info(`🛑 Stopping after user_input - user no longer active`);
                        return null;
                ...
                case "standby":
                    ...
                    const beforeScreenshotPath = this.fileManager.saveScreenshot(pageData.urlHash, this.session.globalStepCounter, "before_standby", beforeScreenshotBuffer);
                    step.beforeScreenshotPath = beforeScreenshotPath;
                    // Execute standby wait
                    const standbyResult = await this.toolStandby(waitTimeSeconds || 5, instruction);
                    ...
                    const afterScreenshotPath = this.fileManager.saveScreenshot(pageData.urlHash, this.session.globalStepCounter, "after_standby", afterScreenshotBuffer);
    ...
    savePageLinkages() {
        ...
        const linkagesObject = Object.fromEntries(Array.from(this.pageLinkages.entries()).map(([source, targets]) => [
            source,
            targets,
        ]));
        ...
        const linkagesData = {
            ...
            totalLinkages: Array.from(this.pageLinkages.values()).reduce((sum, targets) => sum + targets.length, 0),
            linkages: linkagesObject,
        };
        // Save to file system using FileManager pattern
        const linkagesPath = path.join(this.fileManager["sessionDir"], "page-linkages.json");
     ...
     */
    async finalizeSession(objectiveAchieved) {
        ...
        const duration = new Date(this.session.metadata.endTime).getTime() -
    ...
    isUserStillActive() {
        ...
        if (!isActive) {
            logger.info(`🛑 User ${this.userName} is no longer active, stopping exploration`);
        }
        return isActive;
    }
}
