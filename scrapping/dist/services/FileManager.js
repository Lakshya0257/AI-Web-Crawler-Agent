...
export class FileManager {
    ...
    socket;
    userName;
    constructor(sessionId, socket, userName) {
     ...
     */
    static cleanupUserSessions(userName) {
        try {
     ...
     */
    initializeUrlDirectory(urlHash) {
     ...
     * Save page data
     */
    savePageData(urlHash, pageData) {
        const pagePath = path.join(this.sessionDir, "urls", urlHash, "page_data.json");
     ...
     * Save screenshot
     */
    saveScreenshot(urlHash, stepNumber, action, buffer) {
        const filename = `step_${stepNumber.toString().padStart(3, "0")}_${action.replace(/[^a-zA-Z0-9]/g, "_")}.png`;
        const screenshotPath = path.join(this.sessionDir, "urls", urlHash, "screenshots", filename);
     ...
     */
    saveLLMResponse(urlHash, stepNumber, phase, response) {
        const filename = `step_${stepNumber.toString().padStart(3, "0")}_${phase}_response.json`;
        const responsePath = path.join(this.sessionDir, "urls", urlHash, "llm_responses", filename);
        // Create llm_responses directory if it doesn't exist
        const responseDir = path.join(this.sessionDir, "urls", urlHash, "llm_responses");
     ...
     */
    saveDecisionContext(urlHash, stepNumber, context) {
        const filename = `step_${stepNumber.toString().padStart(3, "0")}_decision_context.json`;
        const contextPath = path.join(this.sessionDir, "urls", urlHash, "llm_responses", filename);
        // Create llm_responses directory if it doesn't exist
        const responseDir = path.join(this.sessionDir, "urls", urlHash, "llm_responses");
        fs.mkdirSync(responseDir, { recursive: true });
     ...
     */
    saveSessionConversationHistory(history) {
     ...
     */
    static generateSessionId(baseUrl) {
