...
export class LLMClient {
    ...
    additionalContext;
    canLogin;
    constructor(fileManager, additionalContext, canLogin = false) {
    ...
    getRecentActions(conversationHistory) {
     ...
     */
    async decideNextAction(screenshotBuffer, url, objective, urlHash, stepNumber, conversationHistory, pageQueue, currentSystemStatus, isExplorationObjective, maxPagesReached, userInputs, flowContext, actionHistory) {
        try {
            // ✅ PARSE PREVIOUS ACTION RESULT - Clear boolean and context
            let previousActionContext = "";
            if (conversationHistory.length > 0 &&
                conversationHistory[conversationHistory.length - 1].content.includes("Tool result: ")) {
                try {
                    const lastAction = JSON.parse(conversationHistory[conversationHistory.length - 1].content.split("Tool result: ")[1]);
...
2. THEN: Click on links ONE BY ONE in separate decisions to register new pages for future processing
...
Set "isInSensitiveFlow": true ONLY during LOGIN flows:
...
Remember: Break down complex actions into simple steps. One action per step.`;
            ...
            this.fileManager.saveLLMResponse(urlHash, stepNumber, "decision", parsedResponse);
     ...
     */
    async executePageExtract(screenshotBuffer, url, instruction, objective, urlHash, stepNumber, toolExecutionResult) {
        try {
            const resizedImage = await this.resizeImageForClaude(screenshotBuffer);
            const base64Image = resizedImage.toString("base64");
...
Respond with ONLY valid JSON - no other text or formatting.`,
            ...
            const parsedResponse = JSON.parse(cleanedContent);
            this.fileManager.saveLLMResponse(urlHash, stepNumber, "page_extract_objective_check", parsedResponse);
            return parsedResponse;
        }
        catch (error) {
     ...
     */
    async executePageAct(screenshotBuffer, url, instruction, objective, urlHash, stepNumber, toolExecutionResult) {
        try {
            const resizedImage = await this.resizeImageForClaude(screenshotBuffer);
            const base64Image = resizedImage.toString("base64");
...
- Base your analysis on the tool execution result and screenshot comparison`;
            ...
            cleanedContent = cleanedContent.trim();
            const parsedResponse = JSON.parse(cleanedContent);
            this.fileManager.saveLLMResponse(urlHash, stepNumber, "page_act", parsedResponse);
            return parsedResponse;
        }
        catch (error) {
     ...
     */
    async executeUserInput(inputsRequested, inputsReceived, objective, urlHash, stepNumber) {
        try {
            const response = {
            ...
            // Save the response
            this.fileManager.saveLLMResponse(urlHash, stepNumber, "user_input", response);
            return response;
        }
        catch (error) {
     ...
     */
    async formatExtractionResults(screenshots, // All screenshots from current page
    url, objective, urlHash, stepNumber, previousExtractions, // All previous raw extraction data from current page
    previousMarkdowns, // All previous formatted markdown results
    currentExtractionData // Latest extraction data to incorporate
    ) {
        try {
            // Create image content for all screenshots
            const imageContent = await Promise.all(screenshots.map(async (screenshot) => {
...
Provide the ULTIMATE COMPREHENSIVE FINAL REPORT that preserves all previous context while incorporating new findings.`,
            ...
            this.fileManager.saveLLMResponse(urlHash, stepNumber, "format_extraction", { formattedMarkdown: textContent.trim() });
     ...
     */
    async resizeImageForClaude(imageBuffer) {
